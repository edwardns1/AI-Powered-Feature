...
function MediaForm({ editItem, onAdd, onUpdate, onCancelEdit }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (editItem) {
      ...
      let tags = ''
      try {
        tags = await generateTags(fields.title, fields.notes)
      } catch {
        // non-blocking: proceed without tags if API fails
      }
      onAdd({ id: crypto.randomUUID(), ...fields, tags, createdAt: Date.now() })
      setForm(DEFAULT)
    }
  }
